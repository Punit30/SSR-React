...
import { getCourseTitleListQuery } from "../../gql/queries/Course"
import { setCourseTitle } from "../../app/GlobalObjects/store/reducers/Data"

const useCourseTitle = () => {
	const dispatch = useDispatch()
	const { enqueueSnackbar } = useSnackbar()
	const storedData = useSelector((state) => state.dataReducer.courseTitle)
	const [loading, setLoading] = useState(false)

	const [Query] = useLazyQuery(getCourseTitleListQuery, {
		fetchPolicy: "network-only",
		onCompleted: (data) => {
			const list = _.map(data.getCMECourseTitleList, (item) => ({
				id: item.id,
				name: item.title,
			}))
			const sortedData = _.sortBy(list, "name")
			dispatch(setCourseTitle({ data: sortedData, lastUpdated: Date.now() }))
			setLoading(false)
		},
		onError: (error) => {
