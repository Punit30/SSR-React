...
import { getDegreeListQuery } from "../../gql/queries/Degree"
import { setDegree } from "../../app/GlobalObjects/store/reducers/Data"

const useDegree = () => {
	const dispatch = useDispatch()
	const { enqueueSnackbar } = useSnackbar()
	const storedData = useSelector((state) => state.dataReducer.degree)
	const [loading, setLoading] = useState(false)

	const [Query] = useLazyQuery(getDegreeListQuery, {
		fetchPolicy: "network-only",
		onCompleted: (data) => {
			const list = _.map(data.getDegreeList, (item) => ({
				id: item.id,
				name: item.degreeType,
			}))
			const sortedData = _.sortBy(list, "name")
			dispatch(setDegree({ data: sortedData, lastUpdated: Date.now() }))
			setLoading(false)
		},
		onError: (error) => {
