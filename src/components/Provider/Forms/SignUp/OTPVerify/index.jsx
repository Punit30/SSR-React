...
import { editEmailMutation, sendOTPMutation, verifyOTPMutation } from "../../../../../gql/mutations/Auth"
...
import MailChange from "../../../../common/Modals/MailChange"
import _ from "lodash"

function OTPVerifyForm({ id, email }) {
	...
	const [EditEmail] = useMutation(editEmailMutation)
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			try {
				...
				TrackForm({ formId: "provider_signup_otp_verification", variant: "success" })
				dispatch(setProviderSignupStep(2))
			} catch (e) {
				_.forEach(_.get(e, "graphQLErrors", []), ({ message }) =>
					enqueueSnackbar(message, { variant: "mui-alert", color: "error" })
				)
				TrackForm({ formId: "provider_signup_otp_verification", variant: "danger" })
			}
		},
	})
	...
	const handleEditMail = async (newMail) => {
		try {
			await EditEmail({
				variables: { userId: id, emailInput: newMail },
			})
			dispatch(setProviderSignUp({ email: newMail }))
				...
				<Box className="f f-c g8">
					...
					<Box display="inline">
						...
						<Tooltip title="Edit mail">
							<IconButton sx={{ padding: "4px" }} onClick={() => setEditMailModal(true)}>
...
export default connect(mapStateToProps)(OTPVerifyForm)
